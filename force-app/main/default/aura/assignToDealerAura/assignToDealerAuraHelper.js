({
    getEvaluationDetails : function(component, event, helper) {
        ...
        action.setCallback(this, function(response) {
            let evaluation = response.getReturnValue();
            component.set("v.evaluation", evaluation);
            //component.set("v.primaryContact", evaluation.PrimaryContact__c);
            if(evaluation.PrimaryContact__c != undefined && evaluation.PrimaryContact__c != null) {
                helper.getPrimaryContactUser(component, event, helper, evaluation.PrimaryContact__c);
            }
    ...
    assignToDealer : function(component, event, helper) {
        ...
        var selfEval = component.get("v.selfEvaluation");//component.find("selfEval").get("v.checked");

        action.setParams({"evaluationId": evaluation.Id });
        action.setCallback(this, function(response) {
            var actionPlanIds = response.getReturnValue();
            if(actionPlanIds.length > 0) {
                helper.updateOwnerActionPlans(component, event, helper, actionPlanIds, 0, true);
            }
            else{
                if(selfEval) {
    ...
    updateOwnerActionPlans : function(component, event, helper, actionPlanIds, currentIndex, selfEval) {
        ...
        actionPlanObject.Id = actionPlanIds[currentIndex];
        ...
        action.setCallback(this, function(response) {
            if((currentIndex + 1) != actionPlanIds.length) {
                helper.updateOwnerActionPlans(component, event, helper, actionPlanIds, currentIndex + 1, selfEval);
            }
            else {
                if(selfEval) {
                    helper.updateOwnerEvaluation(component, event, helper);

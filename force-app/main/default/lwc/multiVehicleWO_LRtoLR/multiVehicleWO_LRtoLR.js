...
export default class MultiVehicleWO_LRtoLR extends NavigationMixin(LightningElement) {
    ...
    // FOR STIL-128
    @track isOtherVehicleEdit = null;
    @track vehicleRecordId = false;
    ...
    connectedCallback(){
        console.log('Conn Coallabsakck 222!!'); // QuerySelector Considerations - Do not try to access HTML in connectedCallback()
    ...
    handleValueChange(event){
        ...
        if(vehicleId){
            console.log('vehicleId is not null');
    ...
    handleNewScrSaveBtn(event){
        ...
        console.log('CURRENT VALUE OF isOtherVehicleEdit ');
        
        this.isOtherVehicleEdit = this.checkboxVal == true ? this.checkboxVal  : false;
        console.log(this.isOtherVehicleEdit);
        console.log('event' , event ); 
        console.log( event ); 
        console.log( 'this.vehicleOptions');
        console.log(this.vehicleOptions );
        console.log(this.vehicleOptions.make );
        console.log(this.vehicleOptions.year );
        console.log(this.vehicleOptions.model );
        console.log(this.vehicleOptions.modelCode );
        //  originaltarget
        let vehicleId = event.detail.vehicleid;
        ...
        let vehicleYears = '';
        let vehicleModels = '';
        let vehicleModelcodes ='';
        let checkboxVal = this.checkboxVal;

        let vehicleRecordId = this.vehicleRecordId ? this.vehicleRecordId  : null ;

        if(this.checkboxVal == false && (   this.isOtherVehicleEdit == false || this.isOtherVehicleEdit == null  ) ){
            vehicleMakes = Array.prototype.map.call(this.vehicleOptions.make.filter(x => x.isSelected == true), function(item) { return item.value; }).join(";"); 
            vehicleYears = Array.prototype.map.call(this.vehicleOptions.year.filter(x => x.isSelected == true), function(item) { return item.value; }).join(";");
            vehicleModels = Array.prototype.map.call(this.vehicleOptions.model.filter(x => x.isSelected == true), function(item) { return item.value; }).join(";");
            vehicleModelcodes = Array.prototype.map.call(this.vehicleOptions.modelCode.filter(x => x.isSelected == true), function(item) { return item.value; }).join(";");
        /**
        ...
        */
        }else if( this.checkboxVal == true && this.isOtherVehicleEdit == true){// when otherVehicle
            vehicleMakes = this.textValue.makeValue.toString();
            vehicleYears = this.textValue.yearValue.toString();
            vehicleModels = this.textValue.modelValue.toString();
            vehicleModelcodes = this.textValue.modelCodeValue.toString();
        }
        console.log('CHECK VALUES BEFORE SAVE ');
        ...
        console.log('test',vehicleModelcodes);
        if(vehicleModelcodes === null || vehicleModelcodes === 'null' || vehicleModelcodes === ''){
            console.log('vehicleModelcodes',vehicleModelcodes);
            vehicleModelcodes = '';
            //isRequiredFieldValid = false;
        ...
        if(isRequiredFieldValid){
            saveVehicles({woId: this.recordId, makes: vehicleMakes, years: vehicleYears, models: vehicleModels, modelcodes: vehicleModelcodes, textValue: checkboxVal, recordId : vehicleRecordId})
            .then(result => {
    ...
    handleVehicleEdit(event){
        ...
        this.vehicleRecordId = null ;// when normal vehicle,  this.vehicleRecordId become null 
        ...
        if(vehicle){
            ...
            */
            if(this.checkboxVal==true){
                this.textValue.makeValue = vehicle.make.selectedValue.toString();
                this.textValue.modelValue = vehicle.model.selectedValue.toString();
                this.textValue.yearValue = vehicle.formattedYear.toString();
                this.textValue.modelCodeValue = vehicle.modelCode.options[0].value.toString();
            }else{
    ...
    handleOtherVehicleEdit(event){

        
        console.log(event);
        console.log('Other Vehicle Edit is clicked!');


        this.isOtherVehicleEdit = true;
        ...
        let vehicle = this.affectedVehicles.find(vehicle => vehicle.id === event.currentTarget.dataset.id);
        console.log(event.currentTarget.dataset.id); 
        this.vehicleRecordId = event.currentTarget.dataset.id ; 
        console.log('vehicle    ',vehicle);
        console.log(vehicle);
        console.log(`event.currentTarget  ${event.currentTarget}`);
        console.log(event.currentTarget);
        ...
        if(vehicle){
            ...
            */
            if(this.checkboxVal==true){
                ...
                this.textValue.yearValue = vehicle.formattedYear.toString();
                this.textValue.modelCodeValue = vehicle.modelCode.options[0].value.toString();
                this.textValue.vehicleId = vehicle.modelCode.options[0].value.toString();
            }else{
                this.vehicleOptions.make = vehicle.make.options;
                this.vehicleOptions.model = vehicle.model.options;
                this.vehicleOptions.year = vehicle.year.options;
    ...
    */
    valChange(event){
        ...
        console.log('valChange for  txt ');
        console.log( event.target.name );
        console.log( event.target.value );
        console.log(this.template.querySelector('[name="vehicleId"]'));
        // console.log( this.vehicleRecordId  ); 
        // this.vehicleRecordId = this.template.querySelector('[name="vehicleId"]').value;

        if(event.target.name == 'makeValue'){
            this.textValue.makeValue = event.target.value;
         }
        if(event.target.name == 'modelValue'){
            this.textValue.modelValue = event.target.value;
         }
        if(event.target.name == 'yearValue'){
            this.textValue.yearValue = event.target.value;
         }
        if(event.target.name == 'modelCodeValue'){
            this.textValue.modelCodeValue = event.target.value;
         }
        // if(event.target.name == 'vehicleId'){ // 값이 바뀌어서도 안되고 바뀔 일도 없어야 함. 
        //     this.textValue.vehicleId = event.target.value;
        //     this.vehicleRecordId = event.target.value;
        //  }
    }

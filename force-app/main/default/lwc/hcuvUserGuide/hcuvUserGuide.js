...
export default class HcuvUserGuide extends LightningElement {

    @track formsAndGuides = communityList.HCUV_Forms_And_Guides;
    @track salesforce_Training_Video = communityList.HCUV_SALESFORCE_TRAINING_VIDEO;
        ...
        if (result.data) {
            ...
            if (refMaterials.ReferenceMaterials && refMaterials.ReferenceMaterials.length > 0) {
                refMaterials.ReferenceMaterials.forEach((item) => {
                    ...
                    else if (item.Material_Type__c == 'Video' && item.Name == communityList.HCUV_SALESFORCE_TRAINING_VIDEO ) //for User Guide

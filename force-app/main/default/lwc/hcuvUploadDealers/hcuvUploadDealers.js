...
import createDealerGroup from '@salesforce/apex/HCUV_UploadDealersCtrl.createDealerGroup';

export default class HcuvUploadDealers extends LightningElement {
    ...
    createRecords() {
        ...
        this.data.forEach(item => {
            if (item['Dealer Code'] && item['Dealer Code'] != ''
            ) {
        ...
        // call createDealerGroup

        createDealerGroup({ recordId: this.recordId, objectApi: this.objectApiName, data: eligibleData })
            .then(result => {
            ...
            .catch(error => {
                this.showSpinner = false;
                console.log('error in initialize = ' + JSON.stringify(error));
    ...
    parse(csv) {
        ...
        if(!headers || !headers.includes('Dealer Code'))
            this.error = 'File failed to upload. Please check the format and try again.';

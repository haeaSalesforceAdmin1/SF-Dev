...
export default class EvaluationDpmrGmaSales extends NavigationMixin(LightningElement) {
    ...
    // Added by JongHoon Kim 01.04.2025
    @track errorMessage = '';
    @api fileName = '';
    @track fileContent = '';
    filesize = 0;
    ...
    topicOptions = [
        ...
        { label: 'RPB', value: 'RPB' },
        { label: 'Sales Efficiency / Market Share', value: 'Sales Efficiency / Market Share' },
    ...
    loadContacts() {
        ...
        .then(result => {
            //const optionsWithNA = [{ label: 'N/A', value: 'N/A' }, ...result]; //Added to add 'N/A' to seconday contact options by Jonghoon Kim DPM-5904 12.04.2024
            this.contactOptions = result;
            //this.secondaryContactOptions = optionsWithNA; //Added to add 'N/A' to seconday contact options by Jonghoon Kim DPM-5904 12.04.2024
        })
        .catch(error => {
...
uploadFilesSequentially(recordId, files) {
    ...
    files.forEach(file => {
        uploadPromise = uploadPromise.then(() => {
            ...
            if (!file.base64Data || !file.contentType) {
                return Promise.reject(`Invalid file data for: ${file.name}`);
            }
            ...
            return Promise.reject(`Failed to upload file: ${file.name}`);
        });
    });
...
handleUploadFinished(event) {
    ...
    //     let uploadPromise = Promise.resolve(); // Initialized Promise
    ...
    //     return uploadPromise; // Return a Promise after all file uploads are complete
    // }

    handlePrevious() {
        const previousEvent = new CustomEvent('changeflag', {
            detail: {
                nextStep_GMASales: false,
